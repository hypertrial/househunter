...
export function mountainLabel(place: PlaceSummary): string {
  return place.mountain_score === null ? "Unavailable" : `${place.mountain_score.toFixed(1)} /100`;
}
...
function Workspace({ meta }: { meta: Meta }) {
  ...
  const [mountainMin, setMountainMin] = useState<number | null>(initial.mountainMin);
  ...
  const [draftMountainMin, setDraftMountainMin] = useState<number | null>(mountainMin);
  ...
  const writeHash = useCallback((mode: "push" | "replace") => {
    ...
    if (mountainMin !== null) params.set("mountain_min", String(mountainMin));
  ...
  }, [county, level, metric, mountainMin, selected, showUnranked, state]);
  useEffect(() => {
  ...
  useEffect(() => {
    const restore = () => {
      ...
      setMountainMin(parsed.mountainMin); setDraftMountainMin(parsed.mountainMin);
  ...
  async function openExtremes(trigger: HTMLElement) {
    ...
    const sort = community ? "community_conditions_group" : metric === "mountain" ? "mountain_score" : "risk_score";
    ...
    if (mountainMin !== null) base.set("mountain_min", String(mountainMin));
  ...
  function applyFilters() {
    ...
    setState(nextState); setCounty(level === "tract" ? draftCounty : ""); setShowUnranked(draftUnranked); setMountainMin(draftMountainMin); setSelected(""); setDetailTarget(null); setOverlay(null);
