...
import { COMMUNITY_GROUP_COLORS, MAP_COLORS, readHash, scoreBand, STATE_ABBREVIATIONS, STATE_FIPS, type CameraState, type ScoreBand } from "./map";
...
function metricLabel(place: PlaceSummary, metric: Metric): string {
  return metric === "fema" ? scoreLabel(place) : communityLabel(place);
}
...
function Workspace({ meta }: { meta: Meta }) {
  ...
  }, [county, level, metric, selected, showUnranked, state]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const path = detailTarget.level === "county"
      ? `/api/v1/counties/${detailTarget.id}`
      : `/api/v1/places/${detailTarget.id}`;
  ...
  async function openExtremes(trigger: HTMLElement) {
    ...
    const base = new URLSearchParams({ limit: community ? "1" : "5", sort: community ? "community_conditions_group" : "risk_score" });
    ...
    const path = community ? "/api/v1/counties" : level === "tract" ? "/api/v1/places" : "/api/v1/counties";
    try {
      ...
      if (!community) {
        ...
        const samples = async (group: number | null | undefined) => {
          ...
          if (state) params.set("state", state);
          return json<PlacePage>(`/api/v1/counties?${params}`);
        };
  ...
  async function browseCommunity(group: number, offset = 0) {
    ...
    try {
      const page = await json<PlacePage>(`/api/v1/counties?${params}`);
  ...
  function applyFilters() {
    ...
    setState(nextState); setCounty(level === "tract" ? draftCounty : ""); setShowUnranked(draftUnranked); setSelected(""); setDetailTarget(null); setOverlay(null);
  ...
  function clearFilters() { setDraftState(builtState); setDraftCounty(""); setDraftUnranked(false); setState(builtState); setCounty(""); setShowUnranked(false); setFocusTarget(builtState ? { kind: "state", id: builtState, nonce: Date.now() } : null); setSelected(""); setDetailTarget(null); setOverlay(null); }
  ...
  return <main className="map-shell">
    ...
    {overlay === "extremes" && <section id="extremes-panel" className="floating-panel extremes-panel" aria-label={metric === "fema" ? "Lowest and highest risk" : "Best and worst Community Conditions"} aria-busy={extremesLoading}><h2>Explore the range</h2>{browseGroup !== null ? <><button className="secondary" onClick={closeCommunityBrowse}>← Back to groups</button><ResultGroup title={`Group ${browseGroup} · ${number.format(browseTotal)} counties`} items={browseItems} metric={metric} onChoose={choose} /><div className="pager"><button className="secondary" disabled={browseOffset === 0} onClick={() => void browseCommunity(browseGroup, Math.max(0, browseOffset - 50))}>Previous</button><span>{number.format(browseOffset + 1)}–{number.format(Math.min(browseOffset + browseItems.length, browseTotal))} of {number.format(browseTotal)}</span><button className="secondary" disabled={browseOffset + 50 >= browseTotal} onClick={() => void browseCommunity(browseGroup, browseOffset + 50)}>Next</button></div></> : extremesLoading ? <p className="loading-copy" role="status">{metric === "fema" ? "Loading lowest and highest…" : "Loading best and worst groups…"}</p> : <div><ResultGroup title={metric === "fema" ? "Lowest" : `Best present · Group ${lowestGroup} · ${number.format(lowestTotal)} counties`} items={lowest} metric={metric} onChoose={choose} onBrowse={metric === "community-conditions" && lowestGroup !== null ? () => void browseCommunity(lowestGroup) : undefined} /><ResultGroup title={metric === "fema" ? "Highest" : `Worst present · Group ${highestGroup} · ${number.format(highestTotal)} counties`} items={highest} metric={metric} onChoose={choose} onBrowse={metric === "community-conditions" && highestGroup !== null ? () => void browseCommunity(highestGroup) : undefined} /></div>}{searchError && <p role="alert" className="error">{searchError}</p>}</section>}
    ...
    {overlay === "info" && <section id="info-panel" className="floating-panel info-panel" aria-label="About this map"><h2>About this map</h2><p>Every geography is drawn from the pinned FEMA National Risk Index December 2025 release. Dense tracts become distinguishable as you zoom; none are aggregated or enlarged.</p><p>Tracts and counties use separate FEMA percentile universes. County values are never tract averages. Hazard percentiles appear only in details.</p><p>{COMMUNITY_EXPLANATION} Tract colors inherit their county group and never imply tract-level resolution.</p><p>HouseHunter is local-only and uses no basemap, telemetry, account, or hosted database.</p></section>}
    {preview && <div className={tooltipClass} style={{ left: preview.x, top: preview.y }}><strong>{preview.name}</strong><span>{preview.state} · {preview.placeId}</span><b>{metric === "fema" ? preview.score?.risk_score === null || !preview.score ? "Not ranked / unavailable" : `${preview.score.risk_score.toFixed(1)} · ${tooltipBand ? SCORE_BAND_LABELS[tooltipBand] : ""}` : preview.score?.community_conditions_group === null || !preview.score ? "Not grouped · County-level" : `Group ${preview.score.community_conditions_group} of 10 · County-level`}</b></div>}
    ...
    <div className="legend" aria-label={metric === "fema" ? "Score color scale, lower is better" : "Community Conditions groups, Group 1 is healthiest"}><div>{metric === "fema" ? SCORE_BANDS.map((band) => <span key={band.id}><i style={{ background: MAP_COLORS[band.id] }} />{band.label}</span>) : COMMUNITY_GROUP_COLORS.map((color, index) => <span key={color}><i style={{ background: color }} />{index + 1}</span>)}<span><i className="hatched" />{metric === "fema" ? "Unranked" : "Not grouped"}</span></div><p>{metric === "fema" ? <><strong>FEMA ALR_NPCTL</strong> · lower is better · national percentile · not property-level risk</> : <><strong>CHR&R Community Conditions</strong> · 1 healthiest · 10 least healthy · county-level clusters, not percentiles</>}</p></div>
