...
async function installRoutes(
  ...
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    if (url.pathname === "/api/v1/meta") {
      ...
      await route.fulfill({ json: { schema_version: 2, build_id: build.build_id, level: county ? "county" : "tract", scope: build.scope, columns: { place_id: [county ? "08013" : "08013012101"], risk_score: [county ? 18.5 : 21.25], community_conditions_group: [2], mountain_score: [82.5] } } });
...
test("picks polygon holes exactly and resolves overlaps in reverse source order", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/v1/map/scores**", (route) => route.fulfill({ json: {
    schema_version: 2, build_id: build.build_id, level: "tract", scope: build.scope,
    columns: {
      place_id: ["08013012101", "08013012102"], risk_score: [21.25, 35],
      community_conditions_group: [2, 2], mountain_score: [82.5, 82.5],
    },
  } }));
  await page.route("**/api/v1/places/08013012102", (route) => {
...
test("picks a newly exposed state before its replacement bitmap commits", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/v1/map/scores**", (route) => route.fulfill({ json: {
    ...
    columns: {
      ...
      mountain_score: [...exposedCaliforniaIds.map(() => 70), summary.mountain_score],
    },
  } }));
  await page.route(`**/api/v1/places/${selectedId}`, (route) => route.fulfill({ json: detail({
